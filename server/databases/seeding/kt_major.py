...
def run():
    kt_major_subjects = main.get(os.path.join(os.path.dirname(__file__),'clone/KT.txt'))
    query = """
        insert into subjects 
        (nameSubject, codeSubject, stc, major_id)
        values (%s, %s, %s, %s)
        """
    id_major = int(DB().select_data("select id from majors where type = 'kt'", params=None)[0][0])
